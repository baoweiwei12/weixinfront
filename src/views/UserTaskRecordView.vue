<template>
    <div>
        ...
        <el-table :data="taskRecords" style="width: 100%" v-loading="loading">
            <el-table-column prop="group.name" label="群组名" width="130px"></el-table-column>
            <el-table-column prop="personnel.nickname" label="任务人" width="130px"></el-table-column>
            <el-table-column label="内容" width="400px">
                <template slot-scope="scope">
                    ...
                    <audio v-else-if="scope.row.message_type === 34" controls>
                        <source :src="`${baseURL}${scope.row.attched_file}`" type="audio/mpeg">
                        您的浏览器不支持 audio 元素。
                    </audio>
                    <el-tag v-else>{{ messageTypes[scope.row.message_type] }}</el-tag>
                </template>
            </el-table-column>
...
export default {
    ...
    methods: {
        async getWxMessageTypes() {
            this.messageTypes = (await getWxMessageTypes()).data;
        },
